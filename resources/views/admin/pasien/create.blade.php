              ...
              <div class="row">
                  ...
                  <div class="form-group">
                    ...
                    <select name="dokter_id" class="form-control @error('dokter_id') is-invalid @enderror">
                      @foreach ($dokters as $item)
                        <option value="{{ $item->id }}" {{ (old('dokter_id') == $item->id) ? 'selected' : '' }}>{{ $item->nama_dokter }}</option>
                      @endforeach
                    </select>
                    @error('dokter_id')
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <input type="text" name="nama_pasien" class="form-control @error('nama_pasien') is-invalid @enderror" placeholder="Nama Pasien" value="{{ old('nama_pasien') }}">
                    @error('nama_pasien')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                      </span>
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    <label>Keluhan</label>
                    <textarea name="keluhan" class="form-control @error('alamat') is-invalid @enderror" placeholder="Keluhan anda"></textarea>
                    @error('keluhan')
                      <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                      </span>
